...
3. **Task 5.8**: SSL/TLS証明書管理機能実装

### Epic 5: 既存タスクの修正が必要
1. **Task 5.1**: 複数FQDN対応、Attachment Module設定、共有メモリ設定を追加
2. **Task 5.2**: Nginx設定ファイル生成、設定ファイル削除、リトライロジック、起動スクリプトを追加
3. **Task 5.3**: ログローテーション設定を追加
4. **Task 5.6**: 設定取得エージェントのチェック、設定ファイル確認、ヘルスチェックスクリプトを追加

### Epic 4: 新規タスク追加が必要（管理API関連）
1. **Task 4.6**: WAFエンジン向け設定配信API実装（`GET /engine/v1/config`）✅ オプション1で対応
2. **Task 4.7**: WAFエンジン向けAPIトークン管理機能実装
...
- [ ] **Task 5.8**: SSL/TLS証明書管理機能実装

#### 既存タスクの修正が必要（4つ）
- [ ] **Task 5.1**: 複数FQDN対応、Attachment Module設定、共有メモリ設定を追加
- [ ] **Task 5.2**: Nginx設定ファイル生成、設定ファイル削除、リトライロジック、起動スクリプトを追加
- [ ] **Task 5.3**: ログローテーション設定を追加
- [ ] **Task 5.6**: 設定取得エージェントのチェック、設定ファイル確認、ヘルスチェックスクリプトを追加

### Epic 4: 管理画面基本機能実装

#### 新規タスク追加が必要（2つ）
- [ ] **Task 4.6**: WAFエンジン向け設定配信API実装（`GET /engine/v1/config`）✅ オプション1で対応
- [ ] **Task 4.7**: WAFエンジン向けAPIトークン管理機能実装
...
#### 7. SSL/TLS証明書管理機能
**理由**: 設計書（DESIGN.md 3.6.3.2）には「SSL/TLS設定: 各FQDNごとに証明書を設定、Let's Encryptまたは独自証明書、自動更新機能」が記載されているが、タスクに明記されていない

**推奨タスク**:
- **Task 5.8（新規）**: SSL/TLS証明書管理機能実装
  - 各FQDNごとのSSL/TLS証明書設定
  - Let's Encrypt証明書の自動取得・更新機能
  - 独自証明書の設定機能
  - 証明書の有効期限監視
  - 証明書の自動ローテーション

#### 8. ログローテーション設定
**理由**: 設計書（DESIGN.md 3.6.3.3）には「ログローテーション: logrotateで管理」が記載されているが、タスクに明記されていない

**推奨対応**:
- **Task 5.3に追加**: ログローテーション設定実装
  - logrotate設定ファイルの作成
  - Nginxアクセスログのローテーション設定
  - Nginxエラーログのローテーション設定
  - OpenAppSecログのローテーション設定
  - FQDN別ログのローテーション設定（オプション）

#### 9. APIトークン管理機能（管理画面側）
**理由**: 設計書（DESIGN.md 3.1.3.4）には「トークン生成: 管理画面から生成・発行、トークン有効期限: 90日、トークンローテーション」が記載されているが、Epic 4のタスクに含まれているか確認が必要

**推奨対応**:
- **Epic 4に新規タスク追加**: Task 4.7: WAFエンジン向けAPIトークン管理機能実装
  - APIトークン生成・発行機能実装
  - APIトークン一覧表示機能実装
  - APIトークン有効期限管理機能実装（90日）
  - APIトークンローテーション機能実装
  - APIトークン削除・無効化機能実装
  - APIトークン管理画面UI実装

**または、既存タスクに追加**:
- **Task 4.6に追加**: APIトークン管理機能実装（WAFエンジン向け設定配信APIと一緒に実装）

### ✅ 適切にカバーされているタスク
- Task 5.3: ログ転送機能実装（ログローテーション設定を追加推奨）
- Task 5.4: RateLimit機能実装
- Task 5.5: GeoIP機能実装
